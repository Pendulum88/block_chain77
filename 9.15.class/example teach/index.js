...
// const로 list를 초기화한 이유 : 메서드를 호출할때마다 자바스크립트는 해당 코드를 실행하기때문에
// 실행을 최소화 하기 위해 변수에 반환값을 정의하여 이후 다시 찾지않고 바로 쓸수있게 하기 위함

function deleteItem(num) {
  // 삭제를 위한 함수
  list.children[num].classList.add("delete");
  // list의 자식들 중에 num 번째 자식에 delete라는 클래스를 추가한다
  // delete 클래스를 가진 엘리먼트는 css스타일로인해 가운데 줄이 생긴다
  //   let startIdx = 0;
  //   let endIdx = 0;
...
document.getElementById("todo-add").onclick = () => {
  // todo-add, 즉 추가버튼에 클릭에 대한 코드를 정의한다
